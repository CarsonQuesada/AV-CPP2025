...
constexpr uint32_t MAX_PAYLOAD = 64*1024;

enum class MessageID : uint8_t {
    // To Vehicle
    Ping = 0,    // used to notify it is still connected and for ping
    Drive = 1,
    ToggleLights = 2,
    SetMaxSpeed = 4,
    ClientInit = 5,
...
struct StateMode {
    uint8_t mode;  // 0=INIT, 1=MANUAL, 2=ARMING, 3=AUTO, 4=FAULT, 5=ESTOP
};
...
struct StartHeadingCalib {
    // Add fields as needed
};

struct StopHeadingCalib {
    // Add fields as needed
};
...
REGISTER_MESSAGE_ID(StartHeadingCalib, MessageID::StartHeadingCalib)
REGISTER_MESSAGE_ID(StopHeadingCalib, MessageID::StopHeadingCalib)
