...
Autopilot::Autopilot()
	: gps(GPS::get_instance()), lidar(LidarSP::get_instance()), drive(Drive::get_instance()), 
	  adc(ADC::get_instance()), lighting(Lighting::get_instance())
{
}

void Autopilot::toggle_autopilot()
{
	active = !active;
	if (active)
		start();
...
AutopilotOutput Autopilot::run_autopilot()
{
	...
	GPSPoint currPos = gps.get_location();
	GPSPoint dest = destinations.front();
	int lidarDist = lidar.run_lidar();
	int accelVal = 0;
	int brakeVal = 0;
	int steerVal = 0;
...
void Autopilot::find_destinations()
{
...
void Autopilot::clear_destinations()
{
