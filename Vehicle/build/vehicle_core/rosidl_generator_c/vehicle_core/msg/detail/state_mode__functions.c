// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from vehicle_core:msg/StateMode.idl
// generated code does not contain a copyright notice
#include "vehicle_core/msg/detail/state_mode__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
vehicle_core__msg__StateMode__init(vehicle_core__msg__StateMode * msg)
{
  if (!msg) {
    return false;
  }
  // mode
  return true;
}

void
vehicle_core__msg__StateMode__fini(vehicle_core__msg__StateMode * msg)
{
  if (!msg) {
    return;
  }
  // mode
}

bool
vehicle_core__msg__StateMode__are_equal(const vehicle_core__msg__StateMode * lhs, const vehicle_core__msg__StateMode * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // mode
  if (lhs->mode != rhs->mode) {
    return false;
  }
  return true;
}

bool
vehicle_core__msg__StateMode__copy(
  const vehicle_core__msg__StateMode * input,
  vehicle_core__msg__StateMode * output)
{
  if (!input || !output) {
    return false;
  }
  // mode
  output->mode = input->mode;
  return true;
}

vehicle_core__msg__StateMode *
vehicle_core__msg__StateMode__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  vehicle_core__msg__StateMode * msg = (vehicle_core__msg__StateMode *)allocator.allocate(sizeof(vehicle_core__msg__StateMode), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(vehicle_core__msg__StateMode));
  bool success = vehicle_core__msg__StateMode__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
vehicle_core__msg__StateMode__destroy(vehicle_core__msg__StateMode * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    vehicle_core__msg__StateMode__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
vehicle_core__msg__StateMode__Sequence__init(vehicle_core__msg__StateMode__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  vehicle_core__msg__StateMode * data = NULL;

  if (size) {
    data = (vehicle_core__msg__StateMode *)allocator.zero_allocate(size, sizeof(vehicle_core__msg__StateMode), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = vehicle_core__msg__StateMode__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        vehicle_core__msg__StateMode__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
vehicle_core__msg__StateMode__Sequence__fini(vehicle_core__msg__StateMode__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      vehicle_core__msg__StateMode__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

vehicle_core__msg__StateMode__Sequence *
vehicle_core__msg__StateMode__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  vehicle_core__msg__StateMode__Sequence * array = (vehicle_core__msg__StateMode__Sequence *)allocator.allocate(sizeof(vehicle_core__msg__StateMode__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = vehicle_core__msg__StateMode__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
vehicle_core__msg__StateMode__Sequence__destroy(vehicle_core__msg__StateMode__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    vehicle_core__msg__StateMode__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
vehicle_core__msg__StateMode__Sequence__are_equal(const vehicle_core__msg__StateMode__Sequence * lhs, const vehicle_core__msg__StateMode__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!vehicle_core__msg__StateMode__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
vehicle_core__msg__StateMode__Sequence__copy(
  const vehicle_core__msg__StateMode__Sequence * input,
  vehicle_core__msg__StateMode__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(vehicle_core__msg__StateMode);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    vehicle_core__msg__StateMode * data =
      (vehicle_core__msg__StateMode *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vehicle_core__msg__StateMode__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          vehicle_core__msg__StateMode__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!vehicle_core__msg__StateMode__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
